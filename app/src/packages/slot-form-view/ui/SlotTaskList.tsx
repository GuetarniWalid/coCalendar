import { useRef, useCallback, RefObject, useState, useImperativeHandle, forwardRef, useEffect } from 'react';
...
import { computeTaskDiff, TaskDiff, hasPendingChanges } from '../shared/utils/task-diff';
import { savePendingChanges, clearPendingChanges } from '../shared/utils/task-persistence';
import {
  createTask,
  updateTaskText,
  updateTaskDone,
...
export const SlotTaskList = forwardRef<SlotTaskListRef, SlotTaskListProps>(({
    ...
    try {
      ...
      for (const task of diff.deleted) {
        if (!task.id.startsWith('new-task-')) {
          await retryWithBackoff(() => deleteTask(supabase, task.id), 3, 1000);
        }
      }

      for (const { task, changes } of diff.updated) {
        if (task.id.startsWith('new-task-')) continue;

        if (changes.text) {
          await retryWithBackoff(() => updateTaskText(supabase, task.id, task.text), 3, 1000);
        }
        if (changes.is_done !== undefined) {
          await retryWithBackoff(() => updateTaskDone(supabase, task.id, task.is_done), 3, 1000);
        }
        if (changes.position !== undefined) {
          await retryWithBackoff(() => updateTaskPosition(supabase, task.id, task.position), 3, 1000);
        }
      }
      ...
      for (const task of diff.created) {
        const createdTask = await retryWithBackoff(
          () => createTask(supabase, selectedSlot.id, task.text, task.position, task.is_done),
          3,
          1000
        );
  ...
  // Sync with selectedSlot when it changes (e.g., when SlotForm refreshes from cache)
  useEffect(() => {
    if (!selectedSlot?.tasks) {
      return;
    }

    const sortedSlotTasks = [...selectedSlot.tasks].sort((a, b) => a.position - b.position);

    // Compare full task data (ID, text, is_done, position), not just IDs
    const slotTasksHash = sortedSlotTasks.map(t => `${t.id}:${t.text}:${t.is_done}:${t.position}`).join('|');
    const originalTasksHash = originalTasksRef.current.map(t => `${t.id}:${t.text}:${t.is_done}:${t.position}`).join('|');

    // Update if anything changed (IDs, text, is_done, or position)
    if (slotTasksHash !== originalTasksHash) {
      setTasks(sortedSlotTasks);
  ...
  useFocusEffect(
    useCallback(() => {
      // On focus (navigation TO this screen), sync tasks with selectedSlot
      // This handles the case where temp IDs have been replaced with real UUIDs
      if (selectedSlot?.tasks) {
        const sortedSlotTasks = [...selectedSlot.tasks].sort((a, b) => a.position - b.position);

        // Use setTasks with a callback to access current state
        setTasks(currentTasks => {
          // Check if we need to update (different IDs or different length)
          const currentTaskIds = currentTasks.map(t => t.id).join(',');
          const slotTaskIds = sortedSlotTasks.map(t => t.id).join(',');

          if (currentTaskIds !== slotTaskIds) {
            console.log('Syncing tasks on focus - replacing temp IDs with real UUIDs');
            originalTasksRef.current = sortedSlotTasks;
      ...
      // On blur (navigation AWAY), sync to database
      return () => {
  ...
  const handleRemoveTask = useCallback((task: SlotTaskType) => {
    ...
    <View>
      {tasks.length > 0 ? (
        tasks.map((task, index) => {
          ...
          const isNextTaskEmpty = nextTask ? nextTask.text.trim() === '' : false;
